...
// TODO: kalau tabel siswa punya kolom nomor HP ortu/wali (misal: siswa.no_hp atau siswa.no_wali),
// ganti baris di bawah ini jadi ambil dari $d['no_hp'] (hasil query di atas perlu ditambah kolom itu juga).
// Untuk sementara pakai nomor tetap dulu:
$nomor_wa_tujuan = "6282129458031"; // <-- ganti sesuai kebutuhan (format: 62 + nomor tanpa 0 di depan)
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        ...
        .kuitansi {
            ...
            padding: 36px 40px; border-radius: 12px; box-shadow: 0 6px 24px rgba(0,0,0,.08);
        }
        ...
        @media print {
            body { background: #fff; }
            .kuitansi { box-shadow: none; margin: 0; max-width: 100%; border-radius: 0; }
...
<div class="kuitansi">
    ...
    <table class="table table-sm table-borderless mb-0">
        <tr><td class="label">Nama Siswa</td><td>: <strong><?= htmlspecialchars($d['nama']); ?></strong></td></tr>
        <tr><td class="label">NISN / NIS</td><td>: <?= htmlspecialchars($d['nisn'] . ' / ' . $d['nis']); ?></td></tr>
        <tr><td class="label">Kelas</td><td>: <?= htmlspecialchars($d['tingkat'] . ' ' . $jurusan_kode . ' ' . $rombel_no); ?></td></tr>
        <tr>
            ...
            <td>
                ...
                <?php else: ?>
                    : SPP <?= count($daftar); ?> bulan sekaligus, tahun <?= htmlspecialchars($d['tahun_dibayar']); ?>
                    <div class="mt-1">
        ...
        <tr><td class="label">Status</td><td>: <span class="badge bg-success">LUNAS</span></td></tr>
    </table>
...
<div class="text-center mb-4 no-print">
    ...
    <button onclick="kirimWA()" class="btn btn-success">Kirim ke WhatsApp</button>
    <a href="detail_pembayaran.php?ids=<?= implode(',', $daftar_id); ?>" class="btn btn-secondary">Kembali</a>
</div>

<script>
    const nomorWA   = "<?= $nomor_wa_tujuan; ?>";
    const pesanWA   = encodeURIComponent(
        "Assalamu'alaikum, berikut kami sampaikan bukti pembayaran SPP a.n <?= htmlspecialchars($d['nama']); ?> " +
        "sebesar Rp <?= number_format($total_bayar, 0, ',', '.'); ?> dengan status LUNAS. Terima kasih."
    );
    const urlKembali = "detail_pembayaran.php?ids=<?= implode(',', $daftar_id); ?>";

    // Tombol "Kirim ke WhatsApp": buka chat WA lalu balik ke halaman sebelumnya
    function kirimWA() {
    ...
    // Tetap auto-buka dialog cetak begitu halaman siap
